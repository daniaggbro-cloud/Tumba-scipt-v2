...
local function downloadFile(path, func)
    ...
    if not isfile(full_path) then
        ...
        if not suc or res == '404: Not Found' then
            error("Ошибка загрузки " .. path .. ": " .. (res or "Unknown") .. ". URL: " .. url .. ". Проверь имя файла (.lua.txt) и репо public!")
        end
...
if not suc then
    error("Ошибка доступа к репо: " .. BASE_REPO_URL .. " (private или не существует? Сделай public!)")
end
...
-- Загружаем modules/config (с .lua.txt для твоего репо)
